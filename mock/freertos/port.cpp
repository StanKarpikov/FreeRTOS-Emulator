...
static QThread * mod_microControllerSimulationThread = nullptr;
static MicroControllerEnvironment * mod_microControllerSimulation = nullptr;
...
by multiple threads. */
static QRecursiveMutex *pvInterruptEventMutex = NULL;
...
{
    pvInterruptEventMutex = new QRecursiveMutex();
    xPortRunning = pdTRUE;
...
//    SimulatedTask *pvThread;
    ...
    tskTCB* task_tcb = (tskTCB*)pvTaskToDelete;
    xThreadState *task_state = (xThreadState*)(task_tcb->pxTopOfStack);
    SimulatedTask *taskTask = task_state->pvThread;
...
//    pvThread = pxThreadState->pvThread;
	...
	*pxPendYield = pdTRUE;
    ...
    task_state->pvThread = NULL;

	/* Close the thread. */
    delete taskTask; // TODO ???

    mod_microControllerSimulation->removeTask(taskTask);
